...
11.1.0 (14.04.2026)
-------------------
  - В методах класса `Correction` параметр `description` помечен как устаревший (deprecated) и будет полностью удален в следующей мажорной версии
  - Добавлен метод `setPayingAgentInfo`
  - Метод `setPayingAgentInfo` помечен как устаревший (deprecated) и будет полностью удален в следующей мажорной версии. Вместо него рекомендуется использовать `setPayingAgent`
  - Параметр $logger в конструкторе Client теперь явно объявлен как nullable
